...
<template>
    ...
    <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
      <div class="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p class="text-sm text-gray-600 dark:text-gray-400">A list of all the users in your account.</p>
        </div>
          ...
          <CardHeader>
            <div class="flex items-center justify-between">
              ...
              <div class="flex gap-2">
                ...
                <Button
                  v-if="isPrivilegedUser && authUser.id !== user.id"
                  variant="destructive"
                  size="sm"
          ...
          <CardContent>
            <div class="space-y-2">
              ...
              <div class="flex min-h-[30px] flex-wrap gap-2 rounded-md border bg-slate-50 p-2 dark:border-slate-700 dark:bg-slate-800/50">
                <span v-for="role in user.roles" :key="role.id" class="inline-block select-none rounded bg-gray-200 px-2 py-1 text-xs text-gray-700 dark:bg-gray-700 dark:text-gray-200">
                  {{ role.name }}
                </span>
      ...
      <div v-if="!filteredUsers.length" class="rounded-lg border border-dashed bg-gray-50/50 py-12 text-center dark:border-gray-700 dark:bg-gray-800/20 md:col-span-2 lg:col-span-3">
        ...
        <h3 class="mb-2 text-lg font-medium text-gray-900 dark:text-gray-200">No Users to Display</h3>
        <p class="text-gray-500 dark:text-gray-400">There are no users matching your visibility criteria.</p>
      </div>
  ...
  <ConfirmationDialog ref="confirmDialog" title="Delete User" description="Are you sure you want to delete this user? This action cannot be undone." confirm-text="Delete User" cancel-text="Cancel" variant="destructive" icon="⚠️" @confirm="handleDeleteConfirm" />
  <Toast :message="toastMessage" :type="toastType" :show="showToast" @close="handleToastClose" />
</template>
