        ...
        <Card>
          <CardHeader>
            <CardTitle class="flex items-center gap-3">
              <!-- PERBAIKAN: Menambahkan class dark mode -->
              <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-primary/10 dark:bg-primary/20">
                <svg class="h-4 w-4 text-primary dark:text-primary-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <Card class="border-l-4 border-l-primary/20 shadow-sm transition-shadow hover:shadow-md">
                  <CardHeader class="pb-4">
                    <CardTitle class="flex items-center gap-3 text-lg capitalize">
                      <!-- PERBAIKAN: Menambahkan class dark mode -->
                      <div class="flex h-4 w-4 items-center justify-center rounded-full bg-primary/20 dark:bg-primary/30">
                        <div class="h-2 w-2 rounded-full bg-primary dark:bg-primary-400"></div>
                      </div>
                      Available Roles
                  ...
                  <CardContent>
                    <div class="flex flex-wrap gap-3">
                      <template v-for="role in props.allRoles" :key="role.id">
                        <!-- PERBAIKAN: Menambahkan class dark mode untuk state terpilih dan hover -->
                        <label :for="`role-${role.id}`"
                               class="relative flex cursor-pointer items-center gap-3 rounded-lg border px-4 py-2 transition-all"
                               :class="{
                                 'border-primary bg-primary/10 shadow-sm dark:border-primary/80 dark:bg-primary/20': form.roles.includes(role.id),
                                 'border-border/50 hover:border-primary/30 hover:bg-primary/5 dark:hover:border-primary/40 dark:hover:bg-primary/10': !form.roles.includes(role.id)
                               }">
                          <!-- PERBAIKAN: Menambahkan class dark mode -->
